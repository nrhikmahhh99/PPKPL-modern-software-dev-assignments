...
def get_tag(
    tag_id: int = Path(..., gt=0, description="Tag ID"),
    db: Session = Depends(get_db),
) -> TagRead:
...
def patch_tag(
    ...
    if payload.name is not None:
        existing = db.execute(select(Tag).where(Tag.name == payload.name, Tag.id != tag_id)).scalar_one_or_none()
        if existing:
            raise HTTPException(status_code=400, detail="Tag with this name already exists")
